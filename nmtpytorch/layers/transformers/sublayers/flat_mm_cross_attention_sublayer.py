...
from nmtpytorch.utils.mask_utils import generate_default_mask


class FlatMMCrossAttentionSublayer(BaseSublayer):

    def __init__(self, model_dim, n_heads, dropout=0.1, attn_dropout=0.0, is_pre_norm=False):
        """
        ...
        self.multimodal_attn = ScaledDotAttention(model_dim, n_heads, attn_dropout)
    ...
    def forward(self, query, key_txt, value_txt, mask_txt, key_img, value_img, mask_img=None):
        ...
        if value_txt is None:
            value_txt = query
        combined_mask = self._generate_combined_mask(key_img, mask_img, mask_txt)

        multimodal_key = torch.cat((key_img, key_txt), dim=0)
        multimodal_value = torch.cat((value_img, value_txt), dim=0)
        attn_multimodal, attn_weights = self.multimodal_attn((query, multimodal_key, multimodal_value, combined_mask))
