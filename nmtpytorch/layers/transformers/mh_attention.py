...
#     # qk_mask: (batch_size, q_len, 1) x (batch_size, 1, k_len)
    # #           --> (batch_size, q_len, k_len) (binary mask)
    # qk_mask = torch.matmul(
        # src_idxs.ne(0).int().unsqueeze(-1),
        # trg_idxs.ne(0).int().unsqueeze(1),
    # )
    # qq_mask = torch.matmul(
        # src_idxs.ne(0).int().unsqueeze(-1),
        # src_idxs.ne(0).int().unsqueeze(1),
    # )
