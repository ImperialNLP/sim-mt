...
class SIMRL(nn.Module):
    ...
    def init_trans_model(self):
        ...
        dataset = self.model_trans.load_data('train', 32, mode='beam')
    ...
    def setup(self, is_train=True):
        ...
        #ctx_size = self.model_trans.encoders[str(self.sl)].ctx_size
        ################
        # Create Decoder
        ################
        #self.ctx_sizes = self.opts.model['enc_dim']
        ...
        #critic_in_size = 2 + self.opts.model['emb_dim'] + self.opts.model['enc_dim']
    ...
    def forward(self, batch, **kwargs):
        ...
        if self.opts.model['mm_agent_att']:
            ...
            prev_h_image = self.dec.f_init(image_ctx_vec)
            prev_h_image1 = self.enc.f_init(image_ctx_vec)
        else:
            image_ctx_vec = None         
        ...
        while trg_step < max_len:  
            ...
            state_dict_new = torch.zeros(1,batch_length,self.opts.model['enc_dim']).to(DEVICE)
        ...
        critic_loss = self.critic_loss(estimated_values * translations_mask.permute(1,0), cumulative_rewards.to(DEVICE) * translations_mask.permute(1,0))
        ...
        result = {}
        result['loss'] = loss
        #result['loss'] = (loss / seq_len).sum()
        result['n_items'] = 1
        # JI: add critic loss as auxilliary
        self.aux_loss['critic_loss'] = critic_loss
        # self.aux_loss['ent_loss'] = ent_loss
        # self.aux_loss['prep_loss'] = prep_loss
        # self.aux_loss['rewards'] = torch.sum(cumulative_rewards, dim=1).mean()
        return result
