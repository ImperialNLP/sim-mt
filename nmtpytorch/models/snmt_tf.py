...
from nmtpytorch.layers.transformers import TFEncoder, TFDecoder
from nmtpytorch.layers.transformers.visual_object_tags_encoder import VisualObjectTagsEncoder
from nmtpytorch.models import SimultaneousNMT
from nmtpytorch.utils.label_smoothing_loss import LabelSmoothingLoss
...
class SimultaneousTFNMT(SimultaneousNMT):
    ...
    def setup(self, is_train=True):
        ...
        self.loss = LabelSmoothingLoss(trg_vocab_size=self.n_trg_vocab, reduction='sum', ignore_index=0,
                                       with_logits=False)
    ...
    def _create_image_encoder(self):
        if self.opts.model['feat_mode'] is not None:
            return VisualObjectTagsEncoder(
        ...
        # Only for backward compatibility with certain models.
        else:
    ...
    def cache_enc_states(self, batch, **kwargs):
        if self.opts.model["enc_bidirectional"]:
            # It is tricky to cache the encoder's states if it's bidirectional, as they are dependent on future
            # positions due to the self-attention module. Therefore, we are just going to cache the data
    ...
    def forward(self, batch, **kwargs):
        ...
        total_loss = self.loss(result.contiguous().view(-1, result.size(-1)), target_real.contiguous().view(-1))
